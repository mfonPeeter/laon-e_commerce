...
const HeroSection = () => {
  ...
  return (
    <header className="bg-gradient-to-r from-[#ECD8EF] via-[#FFFFFF] to-[#F5F4F7]">
      <div className="container mx-auto flex flex-col space-y-10 items-center mb-4 px-10 pt-10 pb-20 text-center font-lora md:flex-row md:space-y-0 md:space-x-10 md:h-[90vh] md:pb-10 md:pt-0 xl:px-20">
        <div className="md:w-1/2 md:text-left">
          <h1 className="mb-3 max-w-xl mx-auto text-4xl font-raleway font-bold lg:text-5xl lg:mx-0">
            Get <span className="text-blue-700">MI</span> phones at amazing
            prices from Laon's store
          </h1>
        ...
        <div className="relative w-full h-full md:w-1/2 md:flex md:justify-center md:items-end">
          {data.map((obj, index) => (
            <CSSTransition
              key={obj.id}
              mountOnEnter
          ...
          <div className="move-dot-mt flex space-x-3 justify-center mt-[400px]">
            {data.map((obj, index) => (
              <div
                key={obj.id}
